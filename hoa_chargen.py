...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def agilitySkill_valueChanged(self):
        ...
        self.tempagilitySkill = self.agilitySkill.value()
        if self.additional_skill_points == 0:
            if self.rank_not_chosen:
                self.rankBox.setDisabled(False)
            else:
    ...
    def exorcismSkill_valueChanged(self):
        ...
        Add/substract from additional Skill points.
        '''
        self.additional_skill_points += self.tempexorismSkill - self.exorcismSkill.value()
        if self.additional_skill_points >= 0:
            self.additional2Display.setText(str(self.additional_skill_points))
        else:
            self.additional2Display.setText('<span style=" color:#ff0000;">' + str(self.additional_skill_points) + '</span>')
    ...
    def rankBox_changed(self):
        '''
        A nationality was chosen for the character
        '''
        if self.rankBox.currentIndex() == 0:
            self.rank_chosen = ''
            self.languageDisplay.setText('')
            self.deptBox.setDisabled(True)
            self.rank_not_chosen = True
        else:
            self.rank_chosen = self.rank_choice[self.rankBox.currentIndex()]
            self.languageDisplay.setText(self.rank_language[self.rankBox.currentIndex()])
            self.rankBox.setDisabled(True)
            self.deptBox.setDisabled(False)
            self.rank_not_chosen = False
    ...
    def loadButton_clicked(self):
        ...
        if self.filename[0] != '':
            ...
            with open(self.filename[0], 'r') as json_file:
                ...
                self.rank_chosen = self.rank_choice.index(self.temp_field)
                self.rankBox.setCurrentIndex(self.rank_chosen)
    ...
    def saveButton_clicked(self):
        ...
        if self.charnameEdit.text() == '':
            ...
            self.char_data['Rank'] = self.rank_chosen
    ...
    def printButton_clicked(self):
        ...
        pdf.add_page()
        pdf.image(name=CURRENT_DIR + '\\hoa_logo.png')
        pdf.add_font(family='Times New Roman', style='', fname=r'C:\Windows\Fonts\times.ttf', uni='True')
        pdf.add_font(family='Times New Roman', style='B', fname=r'C:\Windows\Fonts\timesbd.ttf', uni='True')
    ...
    def Overview_menu(self):
        ...
        os.startfile(CURRENT_DIR + '\\hoa_chargen_manual.pdf')
...
if __name__ == '__main__':
    ...
    log = logging.getLogger('HOA_Chargen_' + __version__)
    log.setLevel(logging.DEBUG)

    if not os.path.exists('Logs'):
        os.mkdir('Logs')
    
    fh = logging.FileHandler('Logs/hoa_chargen.log', 'w')
